...
// Obtener los libros en el historial del usuario
$sql_libro = "SELECT h.id_libro, l.titulo, l.autor, h.fecha_termino
              FROM historial h
...
// Obtener los libros que deben ser entregados hoy o están a punto de vencer
$sql_entrega = "SELECT h.id_libro, l.titulo, l.autor, h.fecha_termino
                FROM historial h
                INNER JOIN libros l ON h.id_libro = l.id_libro
                WHERE h.id_usuario = '$id_usuario' AND (h.fecha_termino <= CURDATE() + INTERVAL 2 DAY)";
$resultado_entrega = mysqli_query($connection, $sql_entrega);
...
<head>
    ...
    <title>BiblioTecaTeca | Prorrogas</title>
        ...
        <div class="sidebar os-host os-theme-light os-host-resize-disabled os-host-transition os-host-foreign os-host-scrollbar-vertical-hidden os-host-overflow os-host-overflow-x">
          ...
          <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
              ...
              <!-----<li class="nav-item">
                <a href="enviar_recordatorios.php" class="nav-link">
                  <i class="fas fa-info-circle"></i>
                  <p>enviar noti</p>
                </a>
              </li> --->
      ...
      <div class="content-wrapper" style="min-height: 559px;">
        ...
        <div class="content-header">
          <div class="container-fluid">
            <div class="row mb-2">
              <div class="col-sm-6">
                <h1 class="m-0">Prorrogas</h1>
                <h4 class="m-1">La opción renovar se habilita el día antes de la entrega.</h4>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="container-fluid">
            <div class="row">
              <?php 
              while ($row_prorroga = mysqli_fetch_assoc($resultado_entrega)) {
                  $fecha_termino = new DateTime($row_prorroga['fecha_termino']);
                  $hoy = new DateTime(date('Y-m-d')); // Convertir la fecha actual a DateTime
              ?>
              <div class="card">
                  <div class="card-body">
                      <h3 class="card-title"><?php echo $row_prorroga['titulo']; ?></h3>
                      <p class="card-text">Autor: <?php echo $row_prorroga['autor']; ?></p>
                      <h5 class="card-text">Entrega: <?php echo $row_prorroga['fecha_termino']; ?></h5>                     
                      <?php 
                      if ($fecha_termino < $hoy) {
                          echo "<h5>Retrasado</h5>";
                      } else {
                      ?>
                      <form action="renovacion.php?id_libro=<?php echo $row_prorroga['id_libro']; ?>" method="post">
                          <input type="hidden" name="id_libro" value="<?php echo $row_prorroga['id_libro']; ?>">
                          <button type="submit" class="btn btn-danger btn-sm">Renovar</button>
                      </form>
                      <?php } ?>
                  </div>
              </div>
